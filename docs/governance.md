...
- Enable **stdout / JSONL** audit fields from this app (`AUDIT_JSONL_PATH`, structured lines) and forward them to your **SIEM**. Events include workspace lifecycle lines (`workspace.created`, `workspace.group_assigned`, `workspace.spn_assigned`, **`workspace.role_assignment_updated`**) and connection lines. Use **`just cli audit-dump`** to stream the JSONL file to stdout for one-off exports or pipelines (see [Logs and extraction in the root README](https://github.com/calvinchengx/fabric/blob/main/README.md#logs-and-extraction)).
