...
- **Tooling:** [uv](https://docs.astral.sh/uv/) + `pyproject.toml`; lock with `uv lock`; from the repo root **`just sync`** runs `uv sync --all-groups` (see **`justfile`** and [CONTRIBUTING.md](https://github.com/calvinchengx/fabric/blob/main/CONTRIBUTING.md#short-commands-with-just)). **[just](https://github.com/casey/just)** is for **developer** ergonomics (same recipes everywhere); it is **not** something you ask end users to install. For **operators without Python**, ship a **standalone CLI** built with **[PyInstaller](https://github.com/pyinstaller/pyinstaller)** (`fabric-provision.spec`, **`just package-cli`** to rebuild locally — see [CONTRIBUTING.md](https://github.com/calvinchengx/fabric/blob/main/CONTRIBUTING.md#standalone-cli-pyinstaller)).
